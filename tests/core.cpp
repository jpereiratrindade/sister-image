...
int main(){
    ...
    try{
        ...
        appcore::RasterWindowAnalysisConfig c;c.window_size_px=c.min_window_size_px=32;c.compute_anomalies=false;c.ignore_zero_pixels=false;
        c.raster_path=(dir/"strip.tif").string();const auto a=appcore::analyzeRasterWindows(c);
        c.raster_path=(dir/"tile.tif").string();const auto b=appcore::analyzeRasterWindows(c);
        ...
        for(auto mode:{appcore::RasterClassificationMapConfig::Mode::kFeatureClasses,appcore::RasterClassificationMapConfig::Mode::kHomogeneousPatches}){
            appcore::RasterClassificationMapConfig m;m.output_path=(dir/"map.tif").string();m.class_count=2;m.mode=mode;
            appcore::writeRasterClassificationMap(a,m);
        ...
        c.raster_path=(dir/"bad.tif").string();bool rejected=false;try{(void)appcore::analyzeRasterWindows(c);}catch(const std::exception&){rejected=true;}check(rejected,"reject 16 bit");
        c.raster_path=(dir/"strip.tif").string();c.nodata_values={35,210};auto masked=appcore::analyzeRasterWindows(c);check(masked.valid_pixels==2048,"nodata acts on pixels, not window means");
        c.nodata_values={30,40,200,220};masked=appcore::analyzeRasterWindows(c);check(masked.windows.empty()&&masked.ignored_pixels==2048,"nodata ignores all marked values");
        fs::remove_all(dir);std::cout<<"core classification, strip/tile texture and nodata passed\n";return 0;
